...
const getConnection = async (
  ...
  const result = await axios.post<any>(`${USER_API}/connection-list`, {
    ...payload,
    page: pageParam,
  });
...
export const useGetConnectionList = (payload: any) => {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 1 }) => getConnection(payload, pageParam),

    initialPageParam: 1,

    getNextPageParam: (lastPage: {
      data: { pagination: { currentPage: number; totalPages: number } };
    }) => {
